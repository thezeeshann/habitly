...
- **Getty Images Proxy Server** – Set up a proxy server to handle authentication securely
...
<Tabs syncKey="project-type">
  <TabItem label="New Project">
    ...
    Create a new Electron application with Getty Images Editor integration.
    ...
    const GETTY_PROXY_URL = 'https://your-proxy-server.com';
  ...
  <TabItem label="Existing Project">
    ...
    Integrate the Getty Images Editor into an existing Electron application.
...
> **Proxy Server Required:** To use Getty Images in production, you need a proxy server that handles
> authentication. Configure it via the `VITE_GETTY_IMAGES_PROXY_URL` environment
> variable or pass it to `initGettyImagesEditor()`.
...
<CapabilityGrid
  ...
  },
  {
    ...
    description:
      'Authentication is handled securely through a proxy server, keeping credentials safe and separate from the client.',
    imageId: 'client-side',
  },
  {
...
- **Check your proxy server**: Ensure you have set a valid proxy server URL via `VITE_GETTY_IMAGES_PROXY_URL` or passed it to `initGettyImagesEditor()`
- **Check network requests**: Open DevTools Network tab and look for failed requests to your proxy server
- **Verify proxy configuration**: Ensure your proxy server is properly configured to handle Getty Images authentication
