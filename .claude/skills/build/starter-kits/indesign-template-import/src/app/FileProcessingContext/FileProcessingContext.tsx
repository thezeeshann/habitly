/**
 * FileProcessingContext - Manages IDML file processing state
 */
...
const STATUS_MESSAGES: Record<ProcessingStatus, string> = {
  ...
  fetching: 'Loading IDML file...',
  processing: 'Processing IDML file...',
  done: '',
...
export function FileProcessingContextProvider({
  ...
  const processIDMLBlob = useCallback(
    async (blob: Blob, fileName: string) => {
      ...
      try {
        ...
        console.error('Failed to process IDML file:', err);
  ...
  const processUploadedFile = useCallback(
    async (file: File) => {
      ...
      setCurrentFile({
        name: file.name,
        idmlUrl: URL.createObjectURL(file),
        thumbnailBaseUrl: '',
        previewUrl: '',
        alt: file.name
      });
