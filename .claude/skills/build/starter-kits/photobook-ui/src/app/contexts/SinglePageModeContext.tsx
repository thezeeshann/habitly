...
import { zoomToSelectedText, isEqual } from './utils';
 ...
 */
export function SinglePageModeProvider({
  children,
  defaultPaddingTop = 0,
  ...
  useEffect(
    function hideInactivePages() {
      ...
      for (let i = 0; i < sortedPageIds.length; i++) {
        ...
        const shouldBeVisible = pageId === currentPageBlockId;
        if (isValid && shouldBeVisible !== engine.block.isVisible(pageId)) {
          engine.block
            .findAllSelected()
