/**
 * Single Page Focus Hook
 *
 * Manages automatic focus and zoom behavior for single-page editing scenarios.
 * Useful for mobile-optimized UIs where you want to keep the current page in view.
 */
...
interface UseSinglePageFocusOptions {
  ...
  zoomPaddingBottomDefault?: number;
  zoomPaddingRightDefault?: number;
  zoomPaddingLeftDefault?: number;
  verticalTextScrollEnabledDefault?: boolean;
  refocusCropModeEnabledDefault?: boolean;
  textScrollTopPaddingDefault?: number | null;
  textScrollBottomPaddingDefault?: number | null;
}

export function useSinglePageFocus({
  ...
  // With multiple pages in a scene we need to hide other pages
  useEffect(
