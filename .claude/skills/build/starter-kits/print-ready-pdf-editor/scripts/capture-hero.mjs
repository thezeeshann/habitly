...
async function captureHero() {
  ...
  try {
    ...
    const docsAssetsDir = join(__dirname, '..', '..', '..', '..', 'documentation', 'src', 'content', 'docs', 'starterkits', 'print-ready-pdf-editor+prpdf1', 'assets');
    ...
    const lightPngPath = join(assetsDir, 'hero.png');
    const lightWebpPath = join(assetsDir, 'hero.webp');
    ...
    const docsLightWebpPath = join(docsAssetsDir, 'browser.hero.webp');
    await copyFile(lightWebpPath, docsLightWebpPath);
    ...
    const darkPngPath = join(assetsDir, 'hero-dark.png');
    const darkWebpPath = join(assetsDir, 'hero-dark.webp');
    ...
    // Copy to documentation assets
    const docsDarkWebpPath = join(docsAssetsDir, 'browser.hero-dark.webp');
    await copyFile(darkWebpPath, docsDarkWebpPath);
