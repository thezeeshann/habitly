  ...
  async initialize({ cesdk }: EditorPluginContext) {
    ...
    cesdk.actions.register(
      ...
      (options: SetupSceneOptions) => {
        ...
        // Create/update pages by area.id. Pages from previous products that
        // aren't part of the current product's area list are kept alive so
        // their content survives product switches.
        areas.forEach((area) => {
