...
async function captureHero() {
  ...
  try {
    ...
    const assetsDir = join(
      __dirname,
      '../../../documentation/src/content/docs/starterkits/placeholders-editor+ghi789/assets'
    );

    // Ensure assets directory exists
    await mkdir(assetsDir, { recursive: true });

    // Capture light theme (Adopter mode)
    ...
    console.log('Waiting for CE.SDK to initialize...');
    await lightPage.waitForFunction(
      () => window.cesdk && window.cesdk.engine,
      { timeout: 30000 }
    );

    // Switch to Adopter mode (light theme)
    console.log('Switching to Adopter mode...');
    await lightPage.click('button:has-text("Adopter")');
    ...
    await sharp(lightPngPath).webp({ quality: 85 }).toFile(lightWebpPath);
    await unlink(lightPngPath);
    console.log('Light theme screenshot captured successfully!');

    await lightPage.close();

    // Capture dark theme (Creator mode)
    ...
    // Stay in Creator mode (dark theme) - this is the default
    ...
    console.log(`Converting to WebP...`);
    await sharp(darkPngPath).webp({ quality: 85 }).toFile(darkWebpPath);
