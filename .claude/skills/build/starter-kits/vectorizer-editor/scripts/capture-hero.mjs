...
async function captureHero() {
  ...
  try {
    // Start the dev server (using CDN assets for reliable hero capture)
    ...
    // NOTE: Hero images are saved to scripts/hero-images folder.
    // Copy these images to:
    // 1. Starterkit root: ./hero.webp (light theme only)
    // 2. Documentation: apps/documentation/src/content/docs/starterkits/qr-code-editor+qrcde1/assets/
    const assetsDir = join(__dirname, 'hero-images');
    ...
    lightPage.setDefaultTimeout(120000); // 2 minute timeout
    ...
    await lightPage.waitForFunction(() => window.cesdk?.ui, { timeout: 120000 });
    ...
    darkPage.setDefaultTimeout(120000); // 2 minute timeout
    ...
    console.log('Waiting for CE.SDK to load...');
    // Wait for window.cesdk to be available (up to 120 seconds)
    await darkPage.waitForFunction(() => window.cesdk?.ui, { timeout: 120000 });
    console.log('CE.SDK initialized, waiting for scene to stabilize...');
