...
async function captureHero() {
  ...
  try {
    ...
    console.log('Starting dev server on port 5180...');
    serverProcess = spawn('npm', ['run', 'dev:local', '--', '--port', '5180'], {
      stdio: 'pipe',
      shell: true
    });
    ...
    serverUrl = await new Promise((resolve, reject) => {
      ...
      serverProcess.stdout.on('data', (data) => {
        ...
        // Match Vite's local server URL pattern (we expect port 5180)
        const match = output.match(/Local:\s+(http:\/\/localhost:5180)/) || output.match(/Local:\s+(http:\/\/localhost:\d+)/);
        if (match) {
    ...
    const assetsDir = join(__dirname, '../../../documentation/src/content/docs/starterkits/export-using-renderer+exprnd/assets');
