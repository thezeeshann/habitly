...
async function captureHero() {
  ...
  try {
    ...
    const assetsDir = join(__dirname, '../../../documentation/src/content/docs/starterkits/airtable-image-editor+artbl1/assets');
    ...
    // Use 'load' instead of 'networkidle' because Airtable iframe keeps polling
    await lightPage.goto(serverUrl, { waitUntil: 'load', timeout: 60000 });
    ...
    console.log(`Navigating to ${serverUrl}...`);
    // Use 'load' instead of 'networkidle' because Airtable iframe keeps polling
    await darkPage.goto(serverUrl, { waitUntil: 'load', timeout: 60000 });
