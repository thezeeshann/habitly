/**
 * CE.SDK Theming - Color Generation Utilities
 *
 * These functions calculate colors from the CE.SDK configuration
 * into the design token system used by the editor.
 */
...
export function generateColorAbstractionTokensSurface(color: string) {
  return {
    ...
    '--ubq-elevation-1-blur': setLightness(color, '+0.05')
      .alpha(0.85)
      .css('hsl'),
    '--ubq-elevation-2-blur': setLightness(color, '+0.1')
      .alpha(0.85)
      .css('hsl'),
    '--ubq-elevation-3-blur': setLightness(color, '+0.15')
      .alpha(0.85)
      .css('hsl'),
