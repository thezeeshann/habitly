...
async function captureHero() {
  ...
  try {
    ...
    const assetsDir = join(__dirname, '../../../documentation/src/content/docs/starterkits/translation-internationalization+lngde1/assets');
    ...
    await lightPage.waitForFunction(() => window.cesdk && window.cesdk.ui, { timeout: 90000 });
    ...
    console.log('Waiting for CE.SDK to load...');
    // Wait for window.cesdk to be available (up to 90 seconds)
    await darkPage.waitForFunction(() => window.cesdk && window.cesdk.ui, { timeout: 90000 });
    console.log('CE.SDK initialized, waiting for scene to stabilize...');
