...
/**
 * Sets up the background removal plugin with both canvas menu and Apps dock entry.
 *
 * - Canvas menu: Uses official @imgly/plugin-background-removal-web plugin
 * - Apps dock: Custom implementation with @imgly/background-removal
 *
 * Both plugins are dynamically imported to avoid bundling the ~30MB ONNX runtime
 * at build time. This keeps the initial bundle size small and loads the AI
 * models only when background removal is actually used.
 *
