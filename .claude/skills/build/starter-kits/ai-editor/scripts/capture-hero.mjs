...
async function captureHero() {
  ...
  try {
    ...
    // Use headed mode for proper WebGL rendering on macOS
    ...
    // Output to documentation assets directory
    const assetsDir = join(
      __dirname,
      '../../../../documentation/src/content/docs/starterkits/ai-editor+4z6j9l/assets'
    );
    ...
    lightPage.on('console', msg => console.log('Browser:', msg.text()));
    lightPage.on('pageerror', err => console.error('Page error:', err.message));

    console.log(`Navigating to ${serverUrl}...`);
    await lightPage.goto(serverUrl, { waitUntil: 'networkidle', timeout: 60000 });

    // Wait for the editor container to be present
    console.log('Waiting for editor container...');
    await lightPage.waitForSelector('#cesdk_container', { timeout: 60000 });
    ...
    // Listen for console messages and errors
    darkPage.on('console', msg => console.log('Browser:', msg.text()));
    darkPage.on('pageerror', err => console.error('Page error:', err.message));

    console.log(`Navigating to ${serverUrl}...`);
    await darkPage.goto(serverUrl, { waitUntil: 'networkidle', timeout: 60000 });

    // Wait for the editor container to be present
    console.log('Waiting for editor container...');
    await darkPage.waitForSelector('#cesdk_container', { timeout: 60000 });

    console.log('Waiting for CE.SDK to load...');
    // Wait for window.cesdk to be available (up to 90 seconds)
    await darkPage.waitForFunction(() => window.cesdk?.ui, { timeout: 90000 });
    console.log('CE.SDK initialized, waiting for scene to stabilize...');
